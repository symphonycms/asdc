<?php
	...
	Final Class DatabaseUtilities{
	 ...
	 Class ASDCMySQLResult extends DatabaseResultIterator{
		...
		public function current(){
			if($this->_lastPosition != NULL && $this->position() != ($this->_lastPosition + 1) && is_resource($this->_result)){
	...
	Class ASDCMySQL extends Database{
	    ...
	    public function affectedRows(){
	        return ($this->connected()) ? mysql_affected_rows($this->_connection) : 0;
	    }
	    ...
	    public function close(){
			if($this->connected()) mysql_close($this->_connection);
